...
//                [self showResultUI];
                ...
                switch (self.type) {
                    ...
                    case 2:
                        ...
                        DLog(@"self.dataArray2===%@",self.dataArray2);
                        break;
                    case 3:
                        [self.dataArray3 addObjectsFromArray:[dictionary objectForKey:@"data"]];
                        DLog(@"self.dataArray3===%@",self.dataArray3);
                        break;
                    default:
                        break;
                }
...
- (void)footerRequestForService
{
    ...
    NSDictionary *parameters = @{@"isuse":@"0",@"name":user.uid,@"sn":@"",@"pagesize":@"10",@"pageindex":[NSString stringWithFormat:@"%ld",(unsigned long)pageIndex]};
...
- (void)deleteService:(NSString *)subscribeid indexPath:(NSIndexPath *)indexPath
{
    ...
    [manager GetWithParameters:parameters method:@"set.fitness.subscribedelete" success:^(AFHTTPRequestOperation *operation, id responseObject) {
        NSString *resposeString ;
        if ([responseObject isKindOfClass:[NSData class]]) {
            resposeString = [[NSString alloc]initWithData:responseObject encoding:NSUTF8StringEncoding];
        }else
